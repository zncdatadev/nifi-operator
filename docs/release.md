...
This document describes the standard release process for nifi-operator.
...
2. **Docker image** — uses `VERSION` directly:
   `quay.io/zncdatadev/nifi-operator:<version>`

The `VERSION` in `Makefile` are development-time defaults (`0.0.0-dev`) on
`main`. They do not need to be updated for a release.
...
- Docker image is available at
  `quay.io/zncdatadev/nifi-operator:x.y.z-dev`
...
nifi-operator follows [Semantic Versioning](https://semver.org/):
